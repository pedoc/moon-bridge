...
Visual 扩展内部执行的 `visual_brief` / `visual_qa` Provider 调用也会进入 session 计费和 metrics 记录；这类记录的 `model` 使用视觉 Provider 模型 key（例如 `vision/kimi-vision`），`usage_source` 为 `visual_tool_call`。
